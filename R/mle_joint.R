mle_joint <-
function(alpha0,beta0,theta0,delta0,x,y,R,S,family,exposure=rep(1,length(y)),sd.error=FALSE){
   p<-ncol(R)
   q<-ncol(S)
    foo<-function(para,x,y,R,S,family,exposure){
	 p<-ncol(R)
    	q<-ncol(S)
        alpha<-para[1:p]
	beta<-para[(p+1):(p+q)]
	...
	return(loglik_joint(alpha,beta,theta,delta,x,y,R,S,family,exposure))
	}
	para0<-c(alpha0,beta0,theta2z(theta0,family),log(delta0))	
	dummy<-optim(para0,foo,x=x,y=y,R=R,S=S,family=family,exposure=exposure,method="BFGS",hessian=sd.error)
	...
	loglik<-loglik_joint(alpha,beta,theta,delta,x,y,R,S,family)
    mu<-as.vector(exp(R%*%alpha))
    lambda<-as.vector(exp(S%*%beta)*exposure)
    ll<-log(density_joint(x,y,mu,delta,lambda,theta,family))
