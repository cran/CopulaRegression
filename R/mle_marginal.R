mle_marginal <-
function(x,y,R,S,family,exposure=rep(1,length(y)),sd.error=FALSE){
    # fit gamma-model
    n<-length(x)
    ...
    loglik<-loglik_joint(alpha,beta,0,delta,x,y,R,S,1,exposure)
	loglik.ifm<-loglik_joint(alpha,beta,theta.ifm,delta,x,y,R,S,family,exposure)
mu<-as.vector(exp(R%*%alpha))
    lambda<-as.vector(exp(S%*%beta)*exposure)
    ll<-log(density_joint(x,y,mu,delta,lambda,0,1))
    ll.ifm<-log(density_joint(x,y,mu,delta,lambda,theta.ifm,family))
