simulate_joint <-
function(n,mu,delta,lambda,theta,family,max.y=5000,eps=0.00001){
    # simulate gamma-distributed observations
    x<-rgam(n,mu,delta)
    y<-vector(length=n)
    for (i in 1:n){
    stop=FALSE
    upper=max(50,floor(lambda))
    while (stop==FALSE){
    # compute probabilities for 1:upper
    my.prob<-density_conditional(x=x[i],y=1:upper,mu,delta=delta,lambda,theta=theta,family=family)
    # check if the probability for Y=upper is small enough or if upper is larger than the maximal number 
    if (upper>max.y |my.prob[upper]<=eps) {stop=TRUE} else {upper=upper*5}
    }
    #cat(paste("upper: ",upper,"\n"))
    y[i]<-sample(1:upper,1,prob=my.prob)
    }
    Z=cbind(x,y)
    colnames(Z)=c("x","y")
    return(Z)
}
